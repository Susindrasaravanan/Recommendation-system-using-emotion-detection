...
app.config['MONGO_URI'] = 'mongodb://localhost:27017/emowell'  # Replace with your MongoDB URI#for login 
mongo = PyMongo(app)

# Load models and data
face_classifier = cv2.CascadeClassifier(r'project\haarcascade_frontalface_default.xml')
classifier = load_model(r'project\model.h5')
emotion_labels = ['Angry', 'Disgust', 'Fear', 'Happy', 'Neutral', 'Sad', 'Surprise']

# Load data from CSV files
def load_csv_data():
    try:
        movies_df = pd.read_csv(r'project\movies.csv')
        music_df = pd.read_csv(r'project\music.csv')
        games_df = pd.read_csv(r'project\games.csv')
        novels_df = pd.read_csv(r'project\novels.csv')
